...
ind_boundary_unravel = np.squeeze(np.where(
	(Xu == x[0]) | (Xu == x[Nx - 1]) | (Yu == y[0]) | (Yu == y[Ny - 1])))  # outer boundaries 1D unravel indices
ind_boundary = np.where((X == x[0]) | (X == x[Nx - 1]) | (Y == y[0]) | (Y == y[Ny - 1]))  # outer boundary

ind_boundary2_unravel = np.squeeze(
	np.where((Xu > xb2[0]) & (Xu < xb2[1]) & (Yu > yb2[0]) & (Yu < yb2[1])))  # inner boundaries defined by xb2 and yb2
